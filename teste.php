                  ...
                  <?php 
                    for ($i = 1; $i <= count($questoes)+1; $i++) {
                      echo "<div class='funkyradio' id='question$i' style='display: none;'>\n";
                        echo "<h3>" . $questoes[$i-1]["pergunta"] . "</h3>\n";
                        echo "<h3>" . $questoes[$i-1]["cont"] ."</h3>\n";
                        echo "<hr>\n";
                        for ($j = 1; $j <= count($questoes[$i-1]["alternativas"]); $j++) {
                          echo "<div class='funkyradio-success'>\n";
                            echo "<input type='radio' id='opt$i$j' name='opt'/>\n";
                            echo "<label for='opt$i$j'>" . $questoes[$i-1]["alternativas"][$j-1] . "</label>\n";
                          echo "</div>\n";
                        }
                      echo "</div>\n";
                    }
  ...
  <script type="text/javascript">
    ...
    <?php 
      for ($i = 0; $i <= count($questoes); $i++) {
        echo "answers.push(" . $questoes[$i]["resposta"] . ");\n";
      }
    ?>
  </script>
